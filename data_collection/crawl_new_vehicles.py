...
功能：
1. 从 2026年420台新购纯电动公交车辆信息表.csv 中读取车辆
...
# 新购车辆信息文件
NEW_VEHICLES_FILE = "2026年420台新购纯电动公交车辆信息表.csv"
# 珠海公交车辆清单（用于获取线路信息）
...
def get_available_vehicles():
    """从 2026年420台新购纯电动公交车辆信息表.csv 中读取车辆"""
    # 尝试多种编码读取文件
    encodings = ["utf-8-sig", "utf-8", "gb18030", "gbk"]
    ...
    for enc in encodings:
        try:
            with open(NEW_VEHICLES_FILE, "r", encoding=enc, newline="") as f:
                reader = csv.DictReader(f)
                for row in reader:
                    # 从车辆编号列获取车牌号
                    plate = ensure_plate(row.get("车辆编号", ""))
                    if plate:
...
def main():
    ...
    print(f"发现 {len(available_plates)} 辆新购车辆")
    ...
    while True:
        ...
        # 每轮创建新的日志文件
        log_path = os.path.join(OUTPUT_ROOT, f"batch_log_new_vehicles_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
